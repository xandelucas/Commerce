...
namespace Commerce.Test
{
    public class ProdutoServiceTest
    {
        ...
        {
            // Arrange
            var produtoDTO = new ProdutoDTO { Nome = "Produto Atualizado", Valor = 20, Estoque = 10 };
            var produto = new Produto { Nome = "Produto Original", Valor = 10, Estoque = 5 };

            _produtoRepositoryMock.Setup(r => r.AtualizaProdutoAsync(It.IsAny<Produto>())).ReturnsAsync((Produto p) => p);

            // Act
            var result = await _produtoService.AtualizaProdutoAsync(produtoDTO, produto);
        ...
        public async Task AtualizaProdutoAsync_DeveRetornarExcecao_QuandoProdutoDTONulo()
        {
            ...
            // Act
            var result = async () => await _produtoService.AtualizaProdutoAsync(produtoDTO, produto);
